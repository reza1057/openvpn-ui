...
6. Copy `manage.sh`  from `github.com/mosidev/openvpn_ubuntu_2fa` to `/opt/scripts` 
7. copy `vars` from `github.com/mosidev/openvpn_ubuntu_2fa/files` to `/etc/openvpn/server/easy-rsa`
8. Set `EnableAdmin = false` and `RunMode = prod` in the same file, if you don't need to run BeeGo in development mode and don't need BeeGo admin console to run (in most cases you don't need it).
9. On the first run set `OPENVPN_ADMIN_USERNAME` and `OPENVPN_ADMIN_PASSWORD` environment variables to create admin user with secret password:
```bash
